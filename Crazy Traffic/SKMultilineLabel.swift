...
class SKMultilineLabel: SKNode {
    ...
    init(text:String, labelWidth:Int, pos:CGPoint, fontName:String = "ChalkboardSE-Regular",fontSize:CGFloat = 10,fontColor:SKColor = SKColor.black,leading:Int = 18, alignment:SKLabelHorizontalAlignmentMode = .center, shouldShowBorder:Bool = false)
    {
        self.text = text
    ...
    func update() {
        ...
        for line in lines{
            words.append(line.components(separatedBy: separators))
        }
        //= text.componentsSeparatedByCharactersInSet(separators)
        var l = 0
        var lineCount = 0
        while(l < lines.count){
            var finalLine = false
            ...
            while (!finalLine) {
                ...
                while lineLength < CGFloat(labelWidth)
                {
                    ...
                    else
                    {
                        ...
                        lineString = "\(lineString) \(words[l][wordCount])"
